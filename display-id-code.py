...
def check(m,n,line):
    ...
    if type(m[n]) == dict:
        ...
        if n == "resourceType" and m[n] == "Observation":
            ALL.append({"Id":"","Status":"","Category":"","Code":"","Code1":"","Code2":"","Subject":"","Display":"","Time":"","Value":"","Units":"","Display1":"","Display2":"","Value1":"","Value2":""})
...
window=tk.Tk()
window.title("Id & Code Search")
window.geometry("1000x600+250+15")

label1 = tk.Label(window, text = 'id')
label1.pack()
var1 = tk.StringVar()
entry1 = tk.Entry(window, textvariable=var1,width = 40 )
entry1.pack()
label1=tk.Label(window)
label1.pack()

label2 = tk.Label(window, text = 'code')
label2.pack()

var2 = tk.StringVar()
entry2 = tk.Entry(window, textvariable=var2,width = 40 )
entry2.pack()
...
word=tree["columns"]=("Status","Category","Code","Subject","Display","Time","Value","Units")
...
def bt():
    global relation,k1,total,Patient_id
    Patient_id = var1.get()
    code = var2.get()
    #url = "https://oauth.dicom.org.tw/fhir/Observation?code="+code
    url = "http://192.168.50.3:10610/gateway/fhir/Observation?code="+code+"&subject="+Patient_id
...
def main():
    ...
    for i in ALL:
        if i["Code"] == "35094-2":
            ALL.insert(ALL.index(i)+1,{"Id":i["Id"],"Status":i["Status"],"Category":i["Category"],"Code":i["Code1"],"Subject":i["Subject"],"Display":i["Display1"],"Time":i["Time"],"Value":i["Value1"],"Units":i["Units"]})
            ALL.insert(ALL.index(i)+2,{"Id":i["Id"],"Status":i["Status"],"Category":i["Category"],"Code":i["Code2"],"Subject":i["Subject"],"Display":i["Display2"],"Time":i["Time"],"Value":i["Value2"],"Units":i["Units"]})
            ALL.remove(i)
    ...
    for i in range(len(ALL)):
        tree.insert("",i,text=ALL[i]["Id"],values=(ALL[i]["Status"],ALL[i]["Category"],ALL[i]["Code"],ALL[i]["Subject"],ALL[i]["Display"],ALL[i]["Time"],ALL[i]["Value"],ALL[i]["Units"]))
